...
pub struct Order {
    ...
    pub asset: String,
    pub side: OrderSide,
    /// Which account placed this order — needed so a fill against a
    /// resting order settles into the correct owner's wallet/holdings,
    /// not whoever happens to be submitting the current transaction.
    pub user_id: String,
            ...
            OrderSide::Bid => {
                while order.remaining > 1e-9 {
                    ...
                    fills.push(Fill {
                        ...
                        bid_user_id: order.user_id.clone(),
                        ask_order_id: best.id.clone(),
                        ask_price: best.price,
                        ask_user_id: best.user_id.clone(),
                    });
            ...
            OrderSide::Ask => {
                while order.remaining > 1e-9 {
                    ...
                    fills.push(Fill {
                        ...
                        bid_user_id: best.user_id.clone(),
                        ask_order_id: order.id.clone(),
                        ask_price: order.price,
                        ask_user_id: order.user_id.clone(),
                    });
